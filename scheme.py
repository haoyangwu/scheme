...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        child_frame = Frame(self) # Create a new frame with self as the parent
        "*** MY CODE HERE ***"
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    result = scheme_eval(expressions[0], env)
    if scheme_true(result) and expressions.second is nil:
        return result
    elif scheme_false(result):
        return False
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formal, val = binding.first, eval_all(binding.second, env)
...
def check_formals(formals):
    ...
    while rest is not nil:
        if rest.first in parameters:
            raise SchemeError("Repeated parameters")
        elif not scheme_symbolp(rest.first):
            raise SchemeError("{0} is not a symbol".format(rest.first))
        else:
            parameters.append(rest.first)
        rest = rest.second
    return True
